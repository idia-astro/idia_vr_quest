...
class FileBrowserServer implements IFileBrowserServer {
  getImageInfo = async (
    call: ServerUnaryCall<ImageInfoRequest, ImageInfo>,
    callback: sendUnaryData<ImageInfo>
  ) => {
    ...
    try {
      ...
      response.setFiletype(
        magicNum === fitsMagicNumber ? FileType.FITS : FileType.UNKNOWN
      );
  ...
  getFileList = async (
    call: ServerUnaryCall<FileListRequest, FileList>,
    callback: sendUnaryData<FileList>
  ) => {
    const directoryName = call.request.getDirectoryname();
